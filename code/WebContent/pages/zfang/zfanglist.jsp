...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/zfanglist">
  ...
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="mianji" id="mianji" class="form-control" placeholder="面积大小"/>
	</div>
  </div>
  
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="louceng" id="louceng" class="form-control" placeholder="楼层"/>
	</div>
  </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>租房信息列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/zfangadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td>${c.tel}</td>
<td><a href="<%=path %>/zfangview?id=${c.id}" class="btn-default" target="_blank">查看详情</a></td>
<td>${c.stime}</td>
...
<td> 
 <c:if test="${c.zt=='未审核'}">
 <a href="<%=path %>/zfangupdate?id=${c.id}&zt=已审核&forword=zfanglist" class="btn btn-info">审核</a> |
 </c:if>
 <a href="zfangdel?id=${c.id}&forword=zfanglist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/zfanglist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
