...
<td>
  <c:if test="${c.zt=='未审核'}">
 <a href="<%=path %>/kanfangedit?id=${c.id}&forword=kanfangmy" class="btn btn-default">修改</a> |
 <a href="kanfangdel?id=${c.id}&forword=kanfangmy" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
  </c:if>
 </td>
</tr></tbody>
	       ...
	       function go(index)
           {
               window.location.href="<%=path %>/kanfangmy?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
