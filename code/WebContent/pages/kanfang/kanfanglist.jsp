...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/kanfanglist">



  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="sdate" id="sdate"  class="form-control" placeholder="看房日期"/>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>预约看房列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/kanfangadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td> 
 <c:if test="${c.zt=='未审核'}">
 <a href="<%=path %>/kanfangupdate?id=${c.id}&zt=已审核&forword=kanfanglist" class="btn btn-info">审核</a> |
 </c:if>
 <a href="kanfangdel?id=${c.id}&forword=kanfanglist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/kanfanglist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
