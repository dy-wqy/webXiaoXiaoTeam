...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/fanglist">
...
<td> 
 <c:if test="${c.zt=='未审核'}">
 <a href="<%=path %>/fangupdate?id=${c.id}&zt=已审核&forword=fanglist" class="btn btn-info">审核</a> |
 </c:if>
 <a href="fangdel?id=${c.id}&forword=fanglist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/fanglist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
