...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/huserlist">
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>中介列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/huseradd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td> 
 <c:if test="${c.zt=='未审核'}">
 <a href="<%=path %>/huserupdate?id=${c.id}&zt=已审核&forword=huserlist" class="btn btn-info">审核</a> |
 </c:if>
 <a href="huserdel?id=${c.id}&forword=huserlist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/huserlist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
